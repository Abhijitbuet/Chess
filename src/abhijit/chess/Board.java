...
public class Board {
	...
	public Board getObject(int[] x, String[][] piece) {

		String[][] modified = modifyPiece(piece, x[0], x[1], x[2], x[3]);
		final Board b = new Board(modified);
	...
	public double utility() {
		...
		for (int i = 0; i < 8; i++) {
			for (int j = 0; i < 8; i++) {
				if (piece[i][j].startsWith("bl") && (i > 1 && i < 6)
						&& (j > 1 && j < 6))
					val = val + .01;
				else if (piece[i][j].startsWith("w") && (i > 1 && i < 6)
						&& (j > 1 && j < 6))
					val = val - .01;
	...
	public ArrayList<Board> neighbors(String[][] piece, int turn) {
			...
			for (int i = 0; i < 8; i++)
				for (int j = 0; j < 8; j++) {
					if (piece[i][j].startsWith("black")) {
						...
						if (piece[i][j].endsWith("Pawn")) {
							if (i > 1 && i < 7) {
								if (piece[i + 1][j].equals("b")) {
									if (isLegal(i, j, i + 1, j, piece)) {
										int[] temp = new int[4];
										temp[0] = i;
										temp[1] = j;
										temp[2] = i + 1;
									...
									if (piece[i + 1][j + 1].startsWith("w")
											&& j < 7) {
										if (isLegal(i, j, i + 1, j + 1, piece)) {
											int[] temp = new int[4];
											temp[0] = i;
											temp[1] = j;
											temp[2] = i + 1;
								...
								/*
								 * for (int x = 0; x < 8; x++){ /*for (int y =
								 * 0; y < 8; y++) {
								 * 
								 * if (isLegal(i, j, x, y, piece)) { int[] temp
								 * = new int[4]; temp[0] = i; temp[1] = j;
								 * temp[2] = x; temp[3] = y; moves.add(temp);
								 * 
								 * } }
								 * 
								 * }
							...
							if(i==1 && piece[2][j].equals("b") && piece[3][j].equals("b")){
						...
						if(piece[i][j].endsWith("Knight")){
							if (isLegal(i, j, i + 2, j+1 ,
									piece)) {
								int[] temp = new int[4];
								temp[0] = i;
								temp[1] = j;
	...
	public boolean isLegal(int pastX, int pastY, int presentX, int presentY,
		...
		if (pastX == presentX && pastY == presentY)
			...
			return false;
		else if(pastX>7|| pastX<0 || presentX>7|| presentX<0|| pastY>7|| pastY<0 || presentY>7|| presentY<0)return false;
