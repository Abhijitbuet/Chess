...
public class SingleWhite extends Activity {
	int intent = 0;
			...
			for (j = 0; j < 8; j++) {
				...
				image[i][j].setOnTouchListener(new OnTouchListener() {
					...
					case MotionEvent.ACTION_DOWN:
						if(highlightedX!=-1)clear();
						image[pressedX][pressedY].setBackgroundColor(Color.parseColor("#00ff00"));
					...
					default:
						break;
							...
							if (intent == 0) {
								...
								prevX = pressedX;
								prevY = pressedY;
								if (piece[prevX][prevY].equals("b")) {
									intent = 0;
									prevX = pressedX = prevY = pressedY = 0;
								}
								...
								state = isLegal(prevX, prevY, pressedX,
										pressedY, piece);
								if (isWhiteCheckmated(piece)) {
									...
									Board1 b = new Board1(piece);
									Solver s = new Solver();
									int[] sol = s.moveDecider(b, 2);
									prevX=sol[0];
									prevY=sol[1];
									pressedX=sol[2];
									pressedY=sol[3];
									...
									pressedX = pressedY = prevX = prevY = 0;
									intent = 0;
					...
					private void clear() {
						if(base[highlightedX][highlightedY].equals("white"))
							image[highlightedX][highlightedY].setBackgroundColor(Color.parseColor("#ccffff"));
						else
							image[highlightedX][highlightedY].setBackgroundColor(Color.parseColor("#b22222"));
					}
	...
	public ArrayList<Board> getNeighbors(LinkedList<int[]> l) {
		ArrayList<Board> n = new ArrayList<Board>();
		for (int[] a : l) {

			Board b = new Board(piece);
			n.add(b.getObject(a, piece));
		}
		return n;
	}
	...
	public String[][] modifyPiece(String[][] piece, int pastX, int pastY,
		...
		String[][] modified = new String[8][8];
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				modified[i][j] = piece[i][j];
			}
		}
	...
	public void move(int pastX, int pastY, int presentX, int presentY) {
		...
		image[pastX][pastY].setImageDrawable(null);
